...
/**
 * Hook for managing changelog data from GitHub releases
 */
export const useChangelog = () => {
  ...
  const fetchChangelog = useCallback(
    async (packageInfo: PackageInfo, versionInfo: VersionInfo) => {
      setState((prev) => ({
        ...prev,
        package: packageInfo,
        version: versionInfo,
      ...
      try {
        ...
        const changelog = await githubReleases.getChangelogForVersion(
          owner,
          repo,
          versionInfo.version
        );
        ...
        setState((prev) => ({
          ...
          error: changelog
            ? null
            : `No changelog found for version ${versionInfo.version}`,
        }));
      } catch (error) {
        handleError(error);
      }
